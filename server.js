...
app.post('/api/orders/:id/image', (req, res) => {
  ...
  // base64 حداکثر ~13MB برای فایل 10MB — قابل قبول برای JSON ساده
  const order = db.updateOrder(req.params.id, { rxImage: imageData, rxImageName: imageName || 'نسخه.jpg' });
  if (!order) return res.status(404).json({ ok: false, error: 'سفارش پیدا نشد' });
  res.json({ ok: true, order });
});
...
app.post('/api/orders/:id/mark-paid', (req, res) => {
  const { payMethod } = req.body; // 'online' یا 'cod' (پرداخت حضوری)
  const order = db.updateOrder(req.params.id, { status: 'paid', payMethod });
  if (!order) return res.status(404).json({ ok: false, error: 'سفارش پیدا نشد' });
  res.json({ ok: true, order });
});
...
app.post('/api/pharmacist/orders/:id/advance-status', (req, res) => {
  ...
  if (!order) return res.status(404).json({ ok: false, error: 'سفارش پیدا نشد' });
  const STATUS_MSG = {
    preparing: '🧪 داروساز در حال آماده‌سازی سفارش شماست.',
    ready: order.deliver==='pickup' ? '✅ سفارش شما آماده است و می‌توانید برای دریافت آن مراجعه کنید.' : '📦 سفارش شما آماده شد و به پیک تحویل داده می‌شود.',
...
app.post('/api/pharmacist/orders/:id/delivery-stage', (req, res) => {
  ...
  if (!order) return res.status(404).json({ ok: false, error: 'سفارش پیدا نشد' });
  const STAGE_MSG = {
    1: '📦 سفارش شما تحویل پیک شد.',
    2: '🛵 پیک به‌سمت آدرس شما حرکت کرد.',
    3: '✅ سفارش شما تحویل داده شد.'
  };
